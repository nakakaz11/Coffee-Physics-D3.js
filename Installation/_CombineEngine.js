...
var Attraction, Behaviour, Collision, ConstantForce, Demo, EdgeBounce, EdgeWrap, Euler, ImprovedEuler, Integrator, Particle, Physics, Random, Renderer, Spring, Vector, Verlet, Wander, d3SVGRenderer, end, namespace, _ref, _ref1, _ref2,
...
ImprovedEuler = (function(_super) {
  ...
  function ImprovedEuler() {
    _ref1 = ImprovedEuler.__super__.constructor.apply(this, arguments);
    return _ref1;
  }
...
Verlet = (function(_super) {
  ...
  function Verlet() {
    _ref2 = Verlet.__super__.constructor.apply(this, arguments);
    return _ref2;
  }
...
Particle = (function() {
  ...
  Particle.prototype.update = function(dt, index) {
    var behaviour, _i, _len, _ref3, _results;
    if (!this.fixed) {
      _ref3 = this.behaviours;
      _results = [];
...
Physics = (function() {
  ...
  Physics.prototype.integrate = function(dt) {
    var behaviour, drag, index, particle, spring, _i, _j, _k, _len, _len1, _len2, _ref3, _ref4, _ref5, _results;
    drag = 1.0 - this.viscosity;
    _ref3 = this.particles;
    for (index = _i = 0, _len = _ref3.length; _i < _len; index = ++_i) {
      particle = _ref3[index];
      _ref4 = this.behaviours;
...
Collision = (function(_super) {
  ...
  Collision.prototype.apply = function(p, dt, index) {
    var dist, distSq, mt, o, overlap, r1, r2, radii, _i, _len, _ref3, _results;
    _ref3 = this.pool.slice(index);
    _results = [];
    for (_i = 0, _len = _ref3.length; _i < _len; _i++) {
      o = _ref3[_i];
      if (!(o !== p)) {
...
namespace = function(id) {
  var path, root, _i, _len, _ref3, _results;
  root = self;
  _ref3 = id.split('.');
  _results = [];
  for (_i = 0, _len = _ref3.length; _i < _len; _i++) {
    path = _ref3[_i];
    _results.push(root = root[path] != null ? root[path] : root[path] = {});
  }
  return _results;
};
...
Demo = (function() {
  ...
  Demo.COLOURSinit = ["f0f0f0", "d9d9d9", "bdbdbd", "969696", "737373", "525252", "252525", "000000"];

  _swns = {};

  function Demo(globalSwobj, _config) {
    this.mousemove = __bind(this.mousemove, this);
    this.resize = __bind(this.resize, this);
    _swns.opt = globalSwobj;
    _swns.config = _config;
    this.physics = new Physics(null, _config);
...
d3SVGRenderer = (function(_super) {
  var _d3ns, _swns;

  __extends(d3SVGRenderer, _super);

  _swns = {};

  _d3ns = {};

  function d3SVGRenderer(globalSwobj, _config, d3ns) {
    this.setSize = __bind(this.setSize, this);
    d3SVGRenderer.__super__.constructor.apply(this, arguments);
    ...
    this.width = window.innerWidth;
    this.height = window.innerHeight;
    _d3ns.svg = d3.select("#container").append("svg").attr("width", window.innerWidth).attr("height", window.innerHeight);
  }

  d3SVGRenderer.prototype.init = function(physics) {
    d3SVGRenderer.__super__.init.call(this, physics);
    _d3ns.voronoiVertices = [];
    _d3ns.vPath = _d3ns.svg.selectAll(".vPath");
    _d3ns.vLink = _d3ns.svg.selectAll(".vLink");
    _d3ns.edge = function(a, b) {
      return {
        source: a,
        target: b
      };
    };
    _d3ns.dragstarted = function(d) {
      d3.event.sourceEvent.stopPropagation();
      return d3.select(this).classed("dragging", true);
    };
    _d3ns.dragged = function(d) {
      var g;
      g = d3.select(this);
    ...
    _d3ns.dragended = function(d) {
      return d3.select(this).classed("dragging", false);
    };
    _d3ns.drag = d3.behavior.drag().origin(function(d) {
      return d;
    }).on("dragstart", _d3ns.dragstarted).on("drag", _d3ns.dragged).on("dragend", _d3ns.dragended);
    _d3ns.nodes = d3.range(physics.particles.length).map(function(d, i) {
      return {
        radius: physics.particles[i].radius.toFixed(1),
        x: physics.particles[i].pos.x,
        y: physics.particles[i].pos.y,
        swColor: '#' + (physics.particles[i].colour || 'FFFFFF'),
        swText: "　<-|´･_･`) " + i
      };
    });
    ...
    return _d3ns.swPartiCir = _d3ns.svg.selectAll(".swD3c");
  };

  d3SVGRenderer.prototype.render = function(physics) {
    var line, resample, strokes, voronoies;
    d3SVGRenderer.__super__.render.call(this, physics);
    _d3ns.swPartiG.attr("transform", function(d, i) {
      return "translate(" + physics.particles[i].pos.x + "," + physics.particles[i].pos.y + ")";
    }).on("click", function(d, i) {
      var g;
      physics.particles[i].fixed = true;
      physics.particles[i].colour = "BCB11B";
      g = d3.select(this);
      g.append("text").attr("class", "swD3tText2").text(function(d) {
        return _d3ns.nodes[i].swText;
      }).style("font-size", function(d, i) {
        return "" + (d.radius / 1) + "px";
      });
      g.attr("d", function(d) {
        return d.idx = i;
      });
      return g.call(_d3ns.drag);
    });
    _d3ns.swPartiCir.style("fill", function(d, i) {
      return "#" + physics.particles[i].colour;
    }).attr("r", function(d, i) {
      return physics.particles[i].radius.toFixed(1);
    });
    if (this.renderSprings) {
      _d3ns.springsNodes = d3.range(physics.springs.length).map(function(d, i) {
        return {
      ...
      _d3ns.vectPath.attr("x1", function(d) {
        return d.x1;
      }).attr("y1", function(d) {
        return d.y1;
      }).attr("x2", function(d) {
      ...
      _d3ns.vectPath.exit().remove();
    }
    _d3ns.moves = d3.range(physics.particles.length).map(function(d, i) {
      return {
        x: physics.particles[i].pos.x,
        y: physics.particles[i].pos.y
      };
    });
    _d3ns.voronoiVertices = _d3ns.moves.map(function(o) {
      return [o.x, o.y, o];
    });
    switch (true) {
      case _swns.config.vB_hasVoronoi:
        resample = function(points) {
          var i, n, p0, p1, points2, x0, x1, y0, y1;
          i = -1;
          n = points.length;
          p0 = points[n - 1];
          x0 = p0[0];
        ...
        _d3ns.vPath = _d3ns.vPath.data(voronoies);
        _d3ns.vPath.enter().append("path").attr("class", function(d, i) {
          return "vPath vP" + i;
        }).attr("d", function(d, i) {
          return "M" + d.join("L") + "Z";
        }).style({
        ...
        _d3ns.vPath.attr("d", function(d, i) {
          return "M" + d.join("L") + "Z";
        });
        _d3ns.vPath.exit().remove();
        break;
      default:
        null;
    }
    ...
    switch (true) {
      case _swns.config.vB_hasDelaunay:
        strokes = [];
        _d3ns.vLinks = [];
        d3.geom.delaunay(_d3ns.voronoiVertices).forEach(function(d) {
          _d3ns.vLinks.push(_d3ns.edge(d[0], d[1]));
          _d3ns.vLinks.push(_d3ns.edge(d[1], d[2]));
          return _d3ns.vLinks.push(_d3ns.edge(d[2], d[0]));
        });
        _d3ns.vLink = _d3ns.vLink.data(_d3ns.vLinks);
        _d3ns.vLink.enter().append("line").attr("class", function(d, i) {
          return "vLine vL" + i;
        }).style("stroke", function(d, i) {
          var _ref3;
          strokes[i] = (_ref3 = physics.particles[~~(Random(0, physics.particles.length))]) != null ? _ref3.colour : void 0;
          return "#" + strokes[i];
        });
        _d3ns.vLink.attr("x1", function(d) {
          return d.source[2].x;
        }).attr("y1", function(d) {
          return d.source[2].y;
        }).attr("x2", function(d) {
  ...
  d3SVGRenderer.prototype.setSize = function(width, height) {
    this.width = width;
    this.height = height;
    d3SVGRenderer.__super__.setSize.call(this, this.width, this.height);
    return _d3ns.svg.attr("width", this.width).attr("height", this.height);
  };

  d3SVGRenderer.prototype.destroy = function() {
    return $('#container svg').remove();
  };

  return d3SVGRenderer;
